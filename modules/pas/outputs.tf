...
output "ssh_lb_name" {
  value = "${element(concat(google_compute_region_backend_service.ssh_internal.*.name, list("")), 0)}"
}
...
output "tcp_lb_name" {
  value = "${element(concat(google_compute_region_backend_service.tcp_internal.*.name, list("")), 0)}"
}
...
output "lb_name" {
  value = "${element(concat(google_compute_region_backend_service.router_internal.*.name, list("")), 0)}"
}
